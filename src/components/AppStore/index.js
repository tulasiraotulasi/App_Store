...
class AppStore extends Component {
  state = {initialAppsList: tabsList[0].tabId, searchinput: ''}
  ...
  updateCato = tabId => {
    this.setState({initialAppsList: tabId})
  }

  getAllTabs = () => {
    const {initialAppsList} = this.state
    const f = appsList.filter(ele => ele.category === initialAppsList)
    return f
  }

  render() {
    const {initialAppsList, searchinput} = this.state
    const allTabs = this.getAllTabs()
    const finalSearchInput = allTabs.filter(list =>
      list.appName.toLowerCase().includes(searchinput.toLocaleLowerCase()),
    )
        ...
        <ul className="btn">
          {tabsList.map(btn => (
            <TabItem
              key={btn.tabId}
              tabsList={btn}
              isActive={initialAppsList === btn.tabId}
              updateCato={this.updateCato}
            />
          ))}
        </ul>
        <ul className="footer">
          {finalSearchInput.map(tabs => (
            <AppItem key={tabs.appId} appsList={tabs} />
          ))}
        </ul>
      </div>
    )
